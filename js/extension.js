	...
	class Voco extends window.Extension {
	    constructor() {
			...
			this.overlay_poll_done = true;
	      	...
	      	this.content = '';
			this.voco_overlay_el = null;
			this.item_elements = []; //['thing1','property1'];
			this.all_things;
			this.items_list = [];
			this.current_time = 0;
			this.main_site_id = null;
	    ...
	    show() {
			...
			if(text_input_field){
				this.view.querySelector('#extension-voco-text-input-field').addEventListener('keyup', (event) => {
				    if (event.keyCode === 13 && text_input_field.offsetHeight < 100 && this.busy_doing_text_chat_command == false) {
				        if(this.debug){
	                        console.log('voco debug: enter pressed in text command input');
							console.log("voco debug: .offsetHeight: ", text_input_field.offsetHeight);
	                    }
						this.send_input_text();
				    }
				});
				this.view.querySelector('#extension-voco-text-input-field').addEventListener('input', (event) => {
			...
			// CHAT SEND
			this.view.querySelector('#extension-voco-text-input-send-button').addEventListener('click', (event) => {
				if(this.debug){
			...
			// CHAT MORE
			this.view.querySelector('#extension-voco-text-input-more-button').addEventListener('click', (event) => {
				if(this.debug){
			...
			// CHAT SHOW LESS
			this.view.querySelector('#extension-voco-text-input-less-button').addEventListener('click', (event) => {
				if(this.debug){
			...
			// CHAT RESET
			this.view.querySelector('#extension-voco-text-input-reset-button').addEventListener('click', (event) => {
				if(this.debug){
                    console.log("voco debug: reset text chat button clicked. this.llm_assistant_started: ", this.llm_assistant_started);
                }
			...
			this.view.querySelector('#extension-voco-unavailable').addEventListener('click', (event) => {
                this.busy_polling = false;
			});

			this.view.querySelector('#extension-voco-main-controller-not-responding').addEventListener('click', (event) => {
                this.busy_polling = false;
			});
			...
			try{
		          ...
		          `/extensions/${this.id}/api/ajax`,
                    ...
                    if(typeof body.matrix_server != 'undefined' && typeof body.matrix_candle_username != 'undefined'){
                        ...
                        if( body.matrix_server != '...' ){ // && body.has_matrix_token == true
                            ...
                            this.view.querySelector('.extension-voco-matrix-server').innerText = 'https://' + body.matrix_server;
                            this.view.querySelector('.extension-voco-matrix-username').innerText = '@' + body.matrix_username + ':' + body.matrix_server;

                            if(body.matrix_username == '...'){
                                this.view.querySelector('#extension-voco-matrix-download-app-tip').classList.add('extension-voco-hidden');
                            }

                            this.view.querySelector('#extension-voco-matrix-candle-username').innerText = body.matrix_candle_username;
                            this.view.querySelector('#extension-voco-matrix-candle-username-container').classList.remove('extension-voco-hidden');
                        }
                        else{
                            ...
                            this.view.querySelector('#extension-voco-matrix-create-new-account').classList.remove('extension-voco-hidden');
                        }
                    }
                    ...
                    document.getElementById('extension-voco-chat-loading').classList.add('extension-voco-hidden');
		  			...
		  			console.error("Voco: caught error getting Voco Matrix init data: " , err);
		        });
		        ...
		        window.API.postJson(
		          `/extensions/${this.id}/api/init`,
					...
					// Remove spinner
					const loading_spinner_el = this.view.querySelector("#extension-voco-loading");
					if(loading_spinner_el){
						loading_spinner_el.remove();
					}
					
		        }).catch((err) => {
		  			console.error("Voco: caught error getting init data: " , err);
		        });



				this.view.querySelector('#extension-voco-select-satellite-checkbox').addEventListener('change', (event) => {
					//console.log(event);
					...
					try{
				        ...
				        window.API.postJson(
				          `/extensions/${this.id}/api/update`,
							...
							if(this.debug){
                                console.log("Voco debug: update satellite response: ", body);
                            }
							...
							if(body['state'] == true){
								...
								if(is_sat){
									try{
										this.view.querySelector('#extension-voco-content-container').classList.remove('extension-voco-add-token');
                                        this.view.querySelector('#extension-voco-content-container').classList.add('extension-voco-is-satellite');
									}
									catch(e) {
                                        console.error("voco: caught error changing satellite classes: ", e);
                                    }
								}
								else{
									this.view.querySelector('#extension-voco-content-container').classList.remove('extension-voco-is-satellite');
									this.view.querySelector('#extension-voco-select-satellite-checkbox').checked = false;
								}
							}
							else{
								console.log("voco: server reported error while changing satellite state: ", body);
							}


				        }).catch((err) => {
				  			console.error("Voco: caught error changing satellite state: ", err);
				        });

					}
					catch(err){
						console.error("Voco: caught error getting radio buttons value: ", err);
						this.view.querySelector('#extension-voco-select-satellite-checkbox').checked = false;
					}
				...
				console.error("Voco: caught init error: ", err);
			}
			...
			this.interval = setInterval( () => {
				try{
					if( !document.hidden && this.view.classList.contains('selected') ){ // this.llm_assistant_started && 
                        this.do_poll();
						//this.update_text_chat(); // called by parse_poll_response
					}
                    else{
                    ...
                    console.error("Voco: caught error in interval: ", err);
                }
			...
			*/
            ...
            this.view.querySelector('#extension-voco-matrix-create-account-button').addEventListener('click', (event) => {
                ...
                this.view.querySelector('.extension-voco-matrix-server').innerText = server;
                this.view.querySelector('.extension-voco-matrix-username').innerText = '@' + username + ':' + server;
                ...
                this.view.querySelector('#extension-voco-matrix-show-hidden-password-button').classList.remove('extension-voco-hidden');


                this.view.querySelector('#extension-voco-chat-busy-registering').classList.remove('extension-voco-hidden');
            ...
            this.view.querySelector('#extension-voco-matrix-show-new-account-skip').addEventListener('click', (event) => {
                //console.log("skip create matrix account button clicked");
                this.view.querySelector('#extension-voco-matrix-invite-main-user').classList.remove('extension-voco-hidden');
                this.view.querySelector('#extension-voco-matrix-create-new-account').classList.add('extension-voco-hidden');
                this.view.querySelector('#extension-voco-matrix-advanced-tip').classList.add('extension-voco-hidden');
                this.view.querySelector('#extension-voco-matrix-download-app-tip').classList.add('extension-voco-hidden');
            });

            // Join (invite main user) button
            this.view.querySelector('#extension-voco-matrix-invite-main-username-button').addEventListener('click', (event) => {
                ...
                if(invite_username.startsWith('@') && invite_username.indexOf(':') > -1){
                    window.API.postJson(
                      `/extensions/${this.id}/api/ajax`,
            			...
            			if(this.debug){
            				console.log("voco debug: API: create candle account and invite main user response body: ", body);
            			}
            			...
            			if(body['state'] == true){
                            ...
                            this.view.querySelector('#extension-voco-matrix-invite-check-phone-tip').classList.remove('extension-voco-hidden');
                        ...
                        this.view.querySelector('#extension-voco-chat-busy-registering').classList.add('extension-voco-hidden');
              			...
              			console.error("voco: caught error calling create matrix account with invited main user: ", err);
                        this.view.querySelector('#extension-voco-chat-busy-registering').classList.add('extension-voco-hidden');
                        this.view.querySelector('#extension-voco-matrix-create-account-step1').classList.remove('extension-voco-hidden');
                    });
                }
            ...
            this.view.querySelector('#extension-voco-matrix-provide-account-button').addEventListener('click', (event) => {
                //console.log("create matrix save manual account button clicked");

                const server = this.view.querySelector('#extension-voco-matrix-provide-server').value;
                const username = this.view.querySelector('#extension-voco-matrix-provide-username').value;
                const password1 = this.view.querySelector('#extension-voco-matrix-provide-password').value;
            ...
            // Invite new users
            this.view.querySelector('#extension-voco-matrix-invite-username-button').addEventListener('click', (event) => {
                //console.log("clicked on button to invite another user");
                const username = this.view.querySelector('#extension-voco-matrix-invite-username-input').value;
                if(username.startsWith('@') && username.indexOf(':') > 1){
                    window.API.postJson(
                      `/extensions/${this.id}/api/ajax`,
            			...
            			if(body['state'] == true){
                            this.view.querySelector('#extension-voco-matrix-invite-management-output').innerText = "If all goes well an invite should appear within the minute";
            			}
            			else{
            				//console.log("not ok response while getting data");
            				this.view.querySelector('#extension-voco-matrix-invite-management-output').innerText = "Error while inviting user";

            			}

                    }).catch((err) => {
                        console.error('voco: caught error connecting while trying to invite new user: ', err);
                        this.view.querySelector('#extension-voco-matrix-invite-management-output').innerText = "Error while inviting user";
                    });
                }
                else{
            ...
            this.view.querySelector('#extension-voco-matrix-kick-username-button').addEventListener('click', (event) => {
                //console.log("clicked on button to kick a user from the room");
                const username = this.view.querySelector('#extension-voco-matrix-invite-username-input').value;
                if(typeof username == 'string' && username.startsWith('@') && username.indexOf(':') > 1){
                    window.API.postJson(
                      `/extensions/${this.id}/api/ajax`,
                        {'action':'kick',
                        'username':username}
            			...
            			if(body['state'] == true){
                            ...
                            this.view.querySelector('#extension-voco-matrix-invite-management-output').innerText = "The user should now be removed.";
            			}
            			else{
            				//console.log("not ok response while getting data");
                            this.view.querySelector('#extension-voco-matrix-invite-management-output').innerText = "Error while removing user, sorry.";
            			}

                    }).catch((e) => {
                        this.view.querySelector('#extension-voco-matrix-invite-management-output').innerText = "Removing user failed - connection error.";
            ...
            this.view.querySelector('#extension-voco-matrix-invite-refresh-button').addEventListener('click', (event) => {
                //console.log("clicked on button to refresh participants");

                this.view.querySelector('#extension-voco-matrix-invite-refresh-button').classList.add('extension-voco-hidden');

                this.view.querySelector('#extension-voco-matrix-members-list').innerHTML = "";

                setTimeout(function(){
                    this.view.querySelector('#extension-voco-matrix-invite-refresh-button').classList.remove('extension-voco-hidden');
                }, 5000);
                ...
                window.API.postJson(
                  `/extensions/${this.id}/api/ajax`,
        			...
        			//console.log("Refresh request sent, response", body);

                }).catch((e) => {
                    this.view.querySelector('#extension-voco-matrix-invite-management-output').innerText = "Refresh request failed - connection error.";
			...
			this.view.querySelector('#extension-voco-matrix-learn-more-button').addEventListener('click', (event) => {
                //console.log('click on learn more about matrix button');
                //console.log(document.getElementById('extension-voco-matrix-learn-more'));
				this.view.querySelector('#extension-voco-matrix-learn-more').classList.remove('extension-voco-hidden');
                this.view.querySelector('#extension-voco-matrix-learn-more-button').classList.add('extension-voco-hidden');
			});

            // Learn more about Matrix
			this.view.querySelector('#extension-voco-matrix-show-hidden-password-button').addEventListener('click', (event) => {
                //console.log('click on learn more about matrix button');
                //console.log(document.getElementById('extension-voco-matrix-learn-more'));
                this.view.querySelector('#extension-voco-matrix-show-hidden-password-button').style.display = "none";
				this.view.querySelector('#extension-voco-matrix-show-hidden-password-output').innerText = this.matrix_password;
			});
		...
		// General API response parser
		parse_body(body){
            if(typeof body.debug == 'boolean'){
				this.debug = body.debug;
                if(body.debug){
					const debug_warning_el = this.view.querySelector('#extension-voco-debug-warning');
					if(debug_warning_el){
            ...
            if(this.debug){
                console.log("Voco debug: parse_body:  body: ", body);
            }

			
			const voco_content_container_el = this.view.querySelector('#extension-voco-content-container');
			...
			if(voco_content_container_el){
				if('is_satellite' in body){
					if(body['is_satellite']){
						...
						if(voco_content_container_el){
							voco_content_container_el.classList.add('extension-voco-is-satellite');
							this.view.querySelector('#extension-voco-content').classList.remove('extension-voco-show-tab-timers');
							this.view.querySelector('#extension-voco-content').classList.add('extension-voco-show-tab-satellites');
						}
				...
				if('hostname' in body){
					if(body['hostname'] == 'gateway' || body['hostname'] == 'candle'){
						this.view.querySelector('#extension-voco-content-container').classList.add('extension-voco-change-hostname');
					}
					else{
						//console.log(body);
						if('satellite_targets' in body){
							...
							if(Object.keys(body['satellite_targets']).length > 0){
								if(this.debug){
                                    console.log("voco debug: at least one potential main voco controller detected: ", body['satellite_targets']);
                                }
								if('is_satellite' in body){
									//console.log("is_satellite: " + body['is_satellite']);
									if(body['is_satellite']){
										if(this.debug){
                                            console.log("voco debug: I am a satellite");
                                        }
										this.view.querySelector('#extension-voco-select-satellite-checkbox').checked = true;
									}
								}
								this.view.querySelector('#extension-voco-content-container').classList.add('extension-voco-potential-satellite');
								...
								this.view.querySelector('#extension-voco-server-list').innerHTML = list_html;
							}
							else{
								if(this.debug){
                                    console.log("voco debug: satellites length was 0 - no other potential satellites/controllers spotted");
                                }
							}
						}
					}
			...
			if(typeof body['site_id'] == 'string'){
				...
				if(site_id_el){
					...
					let site_id_text = 'Site ID: ' + body['site_id'];
					
					if(typeof body['main_site_id'] == 'string'){
						site_id_text = site_id_text + '<br>Main site ID: ' + body['main_site_id'];
					}
						
					site_id_el.innerHTML = site_id_text;
				}
			...
			try{
				if(this.previous_info_to_show != this.info_to_show){
					...
					if(!this.voco_overlay_el){
						if(this.debug){
							console.log("voco debug: display_info_overlay: adding overlay to document.body");
						}
						this.voco_overlay_el = document.createElement('div');
						this.voco_overlay_el.setAttribute('id','extension-voco-info-overlay');
						document.body.appendChild(this.voco_overlay_el);
					}
				...
				console.error("voco: caught error trying to show big text overlay: ", err);
		...
		do_poll(){
			if(this.debug && location.pathname == '/extensions/voco'){
               console.log("voco debug: in do_poll.  this.busy_polling: ", this.busy_polling);
            }
            if(this.busy_polling){
				this.busy_polling_count++;
                if(this.debug){
                    console.log("voco debug: was still busy polling. this.busy_polling: ", this.busy_polling);
                }
                ...
                if(this.busy_polling_count > 15){
                    ...
                    this.busy_polling_count = 0;
                    if(this.debug){
                        console.log("voco debug: busy polling for over 30 seconds. Resetting this.busy_polling");
                    }
                    this.view.querySelector('#extension-voco-main-controller-not-responding').style.display = 'block';
                    this.view.querySelector('#extension-voco-text-commands-container').style.display = 'none';
                }
                else{
	                if(this.debug){
	                    console.log("voco debug: still busy polling. Aborting poll");
	                }
                    return;
                }
	        ...
	        window.API.postJson(
				...
				if(this.debug && location.pathname == '/extensions/voco'){
					console.log("voco debug: do_poll response: ", body);
				}
                this.parse_poll_response(body);
				this.busy_polling = false;

	        }).catch((err) => {
	  			//console.log("Error getting timer items: " , e);
				if(this.debug){
					console.error("voco debug: caught error doing main poll: ", err);
				}

                this.busy_polling = false;
                this.busy_polling_count = 0;
				
                this.view.querySelector('#extension-voco-unavailable').style.display = 'block';
                this.view.querySelector('#extension-voco-text-commands-container').style.display = 'none';
		...
		parse_poll_response(body){
			...
			const not_responding_warning_el = this.view.querySelector('#extension-voco-main-controller-not-responding');
			if(!not_responding_warning_el){
				console.error("voco poll: missing html? Aborting.");
				return
			}
			not_responding_warning_el.style.display = 'none';


			const list = this.view.querySelector('#extension-voco-list');

			const text_input_field = this.view.querySelector('#extension-voco-text-input-field');
			//const text_chat_container = this.view.querySelector('#extension-voco-text-chat-container');
			const generated_text_output_el = this.view.querySelector('#extension-voco-llm-generated-text-output');
			...
			if(body['state'] == true){
				...
				if(body['is_satellite'] == false){
					if(body['initial_injection_completed']){
						this.view.querySelector('#extension-voco-text-commands-container').classList.remove('extension-voco-hidden');
						
						if(body['busy_starting_snips']){
							this.view.querySelector('#extension-voco-injection-busy').style.display = 'block';
							this.view.querySelector('#extension-voco-text-commands-container').style.display = 'none';
						}
						else{
							this.view.querySelector('#extension-voco-injection-busy').style.display = 'none';
				...
				if(typeof body['text_response'] != 'undefined'){
					if(this.previous_text_chat_response == null){
						...
						if(text_commands_container_el){
							text_commands_container_el.classList.remove('extension-voco-hidden');
						}
					}
				...
				if(typeof body['matrix_started'] != 'undefined'){
					...
					if(body['matrix_started']){
						...
						this.view.querySelector('#extension-voco-chat-loading').classList.add('extension-voco-hidden');
						this.view.querySelector('#extension-voco-matrix-create-account-step1').classList.add('extension-voco-hidden');
						this.view.querySelector('#extension-voco-matrix-create-account-step2').classList.remove('extension-voco-hidden');
					}
					else{
				...
				if( typeof body['matrix_busy_registering'] != 'undefined' && body['matrix_busy_registering'] ){
					if(this.debug){
						console.log('voco debug: matrix is busy registering accounts and starting');
					}
				}
				else{
					this.view.querySelector('#extension-voco-chat-busy-registering').classList.add('extension-voco-hidden');
					...
					if(typeof body['matrix_logged_in'] != 'undefined' & typeof body['matrix_server'] != 'undefined'){
						...
						if(body['matrix_server'] == '...'){
							this.view.querySelector('#extension-voco-chat-loading').classList.add('extension-voco-hidden');
							this.view.querySelector('#extension-voco-matrix-create-account-step1').classList.remove('extension-voco-hidden');
							this.view.querySelector('#extension-voco-matrix-create-account-step2').classList.add('extension-voco-hidden');
						}
						else{
							if(body['matrix_logged_in'] == true){
								...
								this.view.querySelector('#extension-voco-chat-loading').classList.remove('extension-voco-hidden');
							}
			...
			if(typeof body.llm_folder_size != 'undefined'){
				const llm_total_size_el = this.view.querySelector('#extension-voco-llm-total-size')
				if(llm_total_size_el){
					let llm_mb = Math.round(parseInt(body.llm_folder_size)/1000);
					if(llm_mb < 1000){
						llm_total_size_el.textContent = '' + llm_mb + " MB";
					}
					else{
						llm_mb = Math.round(llm_mb / 100) / 10;
						llm_total_size_el.textContent = '' + llm_mb + " GB";
					}

				}

				let dl_indicator_el = this.view.querySelector('#extension-voco-downloading-models-indicator');
				if(dl_indicator_el){
					try{
						...
						console.error("Voco: caught error updating LLM folder size: ", err)
						dl_indicator_el.classList.add('extension-voco-hidden');
					}
			...
			if(typeof body.llm_not_enough_disk_space != 'undefined'){
				const low_disk_el = this.view.querySelector('#extension-voco-main-low-disk-space-warning');
				if(low_disk_el){
			...
			this.busy_polling = false;
			this.busy_polling_count = 0;
			this.view.querySelector('#extension-voco-unavailable').style.display = 'none';
			this.view.querySelector('#extension-voco-text-commands-container').style.display = 'block';


			// TODO: this code is double, also checked doing /llm_init

			let content_container_el = this.view.querySelector('#extension-voco-content-container');
		...
		update_text_chat(){
			if(this.debug){
				console.log("voco debug: in update_text_chat");
				console.log("voco debug: - this.text_chat_response: ", this.text_chat_response);
				console.log("voco debug: - this.previous_text_chat_response: ", this.previous_text_chat_response);
			}
				...
				try{
					...
					if(this.text_chat_messages[i]['timestamp'] < now_stamp - 600000){
						...
						if(old_message_el){
							...
							if(this.debug){
								console.log("voco debug: removed old text chat message");
							}
						}
						else{
						...
						if(this.debug){
							console.log("text message is too old to display: ", this.text_chat_messages[i]);
						}
					}
				}
				catch(err){
					console.error("voco: update_text_chat: caught error looping over old messages: ", err);
				}
			...
			if(this.text_chat_messages.length == 0){
				if(this.debug){
					console.log("voco debug: update_text_chat: stopping because text_chat_messages length is still 0, so no command was been given yet");
				}
				return
			}
			...
			if(this.text_chat_response == null || this.text_chat_response.length == 0){
				if(this.debug){
					console.log("voco debug: update_text_chat: returning. Latest text_chat response is empty. Setting previous_text_chat_response to empty array too");
				}
				this.previous_text_chat_response = [];
				return
			}
			...
			if(JSON.stringify(this.text_chat_response) == JSON.stringify(this.previous_text_chat_response)){
				if(this.debug){
					console.log("voco debug: update_text_chat: returning. text_chat_response and previous_text_chat_response are the same.");
				}
				return
			}
			...
			if(this.previous_text_chat_response == null){
				if(this.debug){
					console.log("voco debug: update_text_chat: returning. previous_text_chat_response was still null. Setting it to text_chat_response instead: ", this.text_chat_response);
				}
			...
			for( let c = 0; c < this.text_chat_response.length; c++){
				...
				if( this.previous_text_chat_response.indexOf(nicer_text) != -1){
					if(this.debug){
						console.log("voco debug: skipping sentence was also in the previous_text_chat_response: ", nicer_text);
					}
					continue
				}
			...
			if(text_chat_container){
				...
				if(message_type == 'command'){
					new_message_el.addEventListener('click', (event) => {
						this.view.querySelector('#extension-voco-text-input-field').value = message;
					});
				}
        ...
        regenerate_matrix_room_members(members){
            try{
                ...
                const list_el = this.view.querySelector('#extension-voco-matrix-members-list');
				if(!list_el){
					return
				}
                list_el.innerHTML = "";
                ...
                for( var m = 0; m < members.length; m++ ){
                    ...
                    if(members[m].user_id != this.matrix_candle_username){
                        ss.addEventListener('click', (event) => {
                            ...
                            this.view.querySelector('#extension-voco-matrix-invite-username-input').value = event.target.innerText;
                        });
                    }
                    else{
				...
				console.error("Voco: caught error in regenerate matrix room members: ", err); // pass exception object to error handler
			}
        }
		...
		regenerate_items(){
			try {
				...
				const list = this.view.querySelector('#extension-voco-list');
				const original = this.view.querySelector('#extension-voco-original-item');
				
				
				if(!list){
					console.error("voco: regenerate items: list el not found?");
					return
				}
				...
				for( var item in items ){
					...
					let owner = '';
					if(this.main_site_id != null){
						owner = this.main_site_id;
						if(typeof items[item][ 'siteId' ] != 'undefined'){
							owner = items[item][ 'siteId' ];
							if(owner != this.main_site_id){
					...
					if( type == 'value' || type == 'boolean_related' ){
						try{
							...
							console.log("voco: caught error handling change data: ", err);
						}
					...
					delete_button.addEventListener('click', (event) => {
						...
						window.API.postJson(
							...
							//console.log(body);
							if( body['state'] != true ){
                                console.log('voco: delete: server responded with error: ', body);
							}

						}).catch((err) => {
							console.error("voco: caught error in save items handler: ", err);
						});
				...
				console.error("voco: caught error in regenerate items: ", err); // pass exception object to error handler
			}
		}
        ...
        show_connected_satellites(connected_satellites, is_satellite){
            try{
                ...
                const list_el = this.view.querySelector('#extension-voco-connected-satellites-list');
                list_el.innerHTML = "";
                ...
                if (recent_sats_count == 0){
                    list_el.innerHTML = "";
                    voco_content_container_el.classList.remove('extension-voco-has-satellites');
                    //document.getElementById('extension-voco-connected-satellites-list-container').classList.add('extension-voco-hidden');
                }
                else{
                    voco_content_container_el.classList.add('extension-voco-has-satellites');
    				...
    				if(is_satellite == true){
    					this.view.querySelector('#extension-voco-select-satellites').style.display = 'block';
    				}
				...
				// statements to handle any exceptions
				console.error("voco: caught error in show_connected_satellites: ", err); // pass exception object to error handler
			}
		...
		update_ai_data(){
	        window.API.postJson(
	          `/extensions/${this.id}/api/ajax`,
				...
				if(this.debug){
                    console.log("voco debug: LLM init response: ", body);
                }
				...
				if(typeof body.llm_tts_not_enough_memory != 'undefined'){
					if(body.llm_tts_not_enough_memory){
						this.view.querySelector('#extension-voco-main-llm-tts-error').textContent = 'Not enough memory';
					}
				}
				if(typeof body.llm_stt_not_enough_memory != 'undefined'){
					if(body.llm_stt_not_enough_memory){
						this.view.querySelector('#extension-voco-main-llm-stt-error').textContent = 'Not enough memory';
					}
				}
				if(typeof body.llm_assistant_not_enough_memory != 'undefined'){
					if(body.llm_assistant_not_enough_memory){
						this.view.querySelector('#extension-voco-main-llm-assistant-error').textContent = 'Not enough memory';
					}
				}
				...
				if(typeof body['controller_pi_version'] != 'undefined'){
					...
					if(this.controller_speed < 5){
						...
						this.view.querySelector('#extension-voco-main-device-model-warning').style.display = 'block';
					}
					if(this.debug){
	                    console.log("voco debug: running on Pi version: ", this.controller_speed);
	                }
				}
				...
				*/
				...
				if(typeof body.llm_models != 'undefined'){
	  			...
	  			console.error("Voco: caught error during llm_init api call: ", err);
	        });
		}
		...
		change_title(new_title){
			if(new_title != this.current_page_title){
				...
				this.view.querySelector('#extension-voco-title').textContent = new_title;
			}
		...
		generate_llm_models_list(llm_models={}){
			if(this.debug){
				console.log("voco debug: in generate_llm_models_list: models: ", llm_models);
			}
			try{
				for (const [llm_type, models] of Object.entries(llm_models)) {
					//console.log("generating models list for: ",llm_type, models)

					let llm_options_list_el = this.view.querySelector('#extension-voco-' + llm_type + '-models-list');
					if(llm_options_list_el){
						...
						for (const [llm_name, llm_details] of Object.entries(llm_models[llm_type]['list'])) {
							...
							if(this.debug){
								console.log("voco debug: model_name: ", model_name, ", llm_details.memory: ", typeof llm_details.memory);
								console.log("voco debug: model_name: ", model_name, ", llm_details.size: ", typeof llm_details.size);
							}
							...
							if(this.debug){
								console.log("voco debug: required_memory: ", typeof required_memory, required_memory);
							}
							...
							if(llm_models[llm_type]['active'] == null){
								...
								if(llm_details.downloaded && model_name != 'voco' && llm_type != 'wakeword'){
									...
									delete_model_button_el.addEventListener('click', (event) => {
										if(this.debug){
											console.log("Voco debug: should delete this model: ", model_name);
										}
								        ...
								        window.API.postJson(
								          `/extensions/${this.id}/api/ajax`,action_dict
											...
											if(this.debug){
							                    console.log('voco debug: delete_llm response: ', body);
							                }
								        }).catch((err) => {
								  			console.error('voco: caught error during delete_llm api call: ', err);
                ...
                console.error("voco: caught error in generate_llm_models_list: ", err);
            }
		}
			...
			if(text_input_field){
		        ...
		        window.API.postJson(
		          `/extensions/voco/api/parse`,
					...
					if(this.debug){
	                    console.log("voco debug: parsing text command response: ", body);
	                }
					if(was_empty){
		...
		reset_to_allow_sending_text_chat(){
			...
			setTimeout(() => {
				...
				this.busy_doing_text_chat_command = false;
				this.view.querySelector('#extension-voco-text-input-send-button').removeAttribute("disabled");
			},2000);
		}
		...
		reset_assistant(){
	        window.API.postJson(
	          `/extensions/${this.id}/api/ajax`,
				...
				if(this.debug){
					console.log("voco debug: llm_reset_assistant response: ", body);
				}
	        }).catch((err) => {
	  			console.error("voco: error during llm_reset_assistant api call: ", err);
	        });
		}
		...
		clear_info_to_show(){
	        window.API.postJson(
	          `/extensions/${this.id}/api/ajax`,
				...
				if(this.debug){
					console.log("voco debug: clear_info_to_show response: ", body);
				}
	        }).catch((err) => {
	  			console.error("Voco: error during clear_info_to_show api call: ", err);
	        });
		}
		...
		llm_generate_text(prompt,llm_action='generate'){
	        window.API.postJson(
	          `/extensions/${this.id}/api/ajax`,
				...
				if(this.debug){
					console.log("voco debug: llm_generate_text response: ", body);
				}

	        }).catch((err) => {
	  			console.error("Voco: error during llm_generate_text api call: ", err);
	        });
		}
