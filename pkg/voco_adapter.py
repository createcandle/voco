...
class VocoAdapter(Adapter):
    ...
    def __init__(self, verbose=True):
        ...
        self.fastest_controller_ip = None
        self.fastest_controller_last_ping_time = 0
        self.main_controller_last_ping_received_time = 0
        ...
        self.llm_last_assistant_query_time = 0
        ...
        if 'aspberry' in self.controller_model:
            self.controller_pi_version = int(self.controller_model.split()[2])
            #print("self.controller_pi_version: " + str(self.controller_pi_version))
        ...
        self.llm_tts_common_responses = [
            ...
            self.main_controller_missing_warning,
        ...
        try:
            ...
            if 'mqtt_server' not in self.persistent_data:
                ...
                self.persistent_data['mqtt_server'] = self.internal_ip
                self.save_to_persistent_data = True
            elif self.persistent_data['mqtt_server'] == 'localhost':
                self.persistent_data['mqtt_server'] = self.internal_ip
                self.save_to_persistent_data = True
            elif self.persistent_data['mqtt_server'] == '0.0.0.0':
                self.persistent_data['mqtt_server'] = self.internal_ip
                self.save_to_persistent_data = True
            elif self.persistent_data['mqtt_server'] == '127.0.0.1':
                self.persistent_data['mqtt_server'] = self.internal_ip
                self.save_to_persistent_data = True
            else:
                if self.DEBUG:
                    print("self.persistent_data['mqtt_server] is not a localhost address: ", self.persistent_data['mqtt_server'])
            ...
            if 'main_controller_ip' not in self.persistent_data: # to remember what the main voco server is, for satellites. Now used to send audio to STT server
                ...
                self.persistent_data['main_controller_ip'] = self.internal_ip   # starts as '127.0.0.1' or 0.0.0.0 or localhost
                self.save_to_persistent_data = True
            ...
            if self.persistent_data['main_controller_ip'] == '127.0.0.1':
                self.persistent_data['main_controller_ip'] = self.internal_ip
                self.save_to_persistent_data = True

            elif self.persistent_data['main_controller_ip'] == 'localhost':
                self.persistent_data['main_controller_ip'] = self.internal_ip
                self.save_to_persistent_data = True
            ...
            # TODO: experiment to no longer store all the satellite thing titles
        ...
        self.multi_things = ['lights','curtains'] # curtains isn't handled yet
        ...
        self.this_is_main_controller = True
        self.metric = True
        self.things = []
        self.groups = []
        self.token = None
        ...
        self.mqtt_connected_to_ip = None
        self.last_run_mqtt_time = 0
        ...
        self.mqtt_last_disconnect_timestamp = 0
        ...
        self.last_time_main_controller_missing_warning_spoken = 0
        ...
        self.hey_snips_wakeword_model_path = os.path.join(self.wakeword_models_dir_path,'hey_snips.tflite')
        ...
        # Assistant
        self.llm_assistant_addon_dir_path = os.path.join(self.addon_dir_path,'llm','assistant')
        self.llm_assistant_data_dir_path = os.path.join(self.llm_data_dir_path, 'assistant')
        os.system('mkdir -p ' + str(self.llm_assistant_data_dir_path))
        ...
        if os.path.exists(str(self.nanotts_path)):
            ...
            print("error, could not find NanoTTS at: " + str(self.nanotts_path))
        ...
        self.llm_assistant_prompt = "The following is a conversation between a curious researcher and their helpful AI assistant called Digital Athena, which gives helpful and honest answers.\\n\\nResearcher:"
        #self.llm_assistant_prompt = "<|im_start|>system \\n A conversation between a user and an LLM-based AI assistant. The assistant gives helpful and honest answers. <|im_end|> \\n <|im_start|>user \\n What is the capital of France? <|im_end|> \\n <|im_start|>assistant \\n Paris is the capital of France. \\n <|im_end|> \\n " #"<|im_start|>user \\n "
        
        # The following is a conversation between a curious researcher and their helpful AI assistant called Digital Athena, which is a large language model trained on the sum of human knowledge.\\n
        ...
        self.llm_assistant_protocols = {
            "basic":{
                ...
                "system_prompt":"The following is a conversation between a curious researcher and their helpful AI assistant called {assistant_name}. The assistant gives helpful and honest answers, and never asks questions.", # Researcher:
            ...
            "chatml":{
                ...
                "system_prompt":"<|im_start|>system \\n A conversation between a user and an LLM-based AI assistant. The assistant gives helpful and honest answers, and never asks questions. <|im_end|>\\n<|im_start|>user \\n What is the capital of France? <|im_end|> \\n <|im_start|>assistant \\n Paris is the capital of France. \\n <|im_end|>",
                "system":"<|im_start|>system\\n{system_prompt}<|im_end|>\\n{reverse_prompt}\\n",
                "user":"{user_message}<|im_end|>",
                "reverse_prompt":"<|im_start|>user",
                "in_prefix":" \\n",
                "in_suffix":"<|im_start|>assistant\\n",
                "end":"<|im_end|>"
            },
            "phi":{
                "user_name":"Alice",
                "assistant_name":"Bob",
                "system_prompt":"The following is a conversation between a curious researcher called Alice and their helpful AI assistant called {assistant_name}, which is a large language model trained on the sum of human knowledge. \\n\\nAlice: What is the capital of Germany?\nBob: Berlin is the capital of Germany.\nAlice:",
                "system":"{system_prompt}\\n\\n{reverse_prompt}",
                "user":"{user_message}\\n",
                "reverse_prompt":"{user_name}:",
                "in_prefix":" ",
        ...
        if self.llm_enabled and (self.llm_stt_enabled or self.llm_assistant_enabled) and self.s == None and self.bits == 64:
...
#
    ...
    def add_from_config(self):
            ...
            if 'System audio volume' in config:
                ...
                if config['System audio volume'] != None:
                    ...
                    if self.system_volume_percentage == 0: # not even possible anymore as the minimum is now set to 10 in manifest.json
                        if self.DEBUG:
                            print("Warning: volume level was set to 0. It will be changed to 85 instead.")
                        self.system_volume_percentage = 85
                    self.set_system_volume_level()
        ...
        try:
            ...
            if 'Assistant threads' in config:
                self.llm_assistant_threads = int(config['Assistant threads'])
                if self.DEBUG:
                    print("-AI assistant thread count was in the config data: " + str(self.llm_assistant_threads))
            

            if 'Do not break up AI assistant sentences' in config:
                self.llm_assistant_do_not_break_up_sentences = bool(config['Do not break up AI assistant sentences'])
                if self.DEBUG:
                    print("-Do not break up AI assistant sentences preference is present in the config data: " + str(self.llm_assistant_do_not_break_up_sentences))
...
#
    ...
    def play_wav(self,file_to_play=None):
        ...
        try:
            ...
            if self.sample_rate != 16000:
                ...
                if self.DEBUG:
                    print("play_wav: resampling took this long: " + str(time.time() - resample_start_time))
    ...
    # DEPRECATED, AND NO LONGER POSSIBLE
    ...
    def aplay(self,file_path, bluetooth=False):
        ...
        if self.DEBUG:
            ...
            print(" - it took this long: " + str(time.time() - aplay_timer))
    ...
    def llm_speak(self,voice_message,intent='default'):
        ...
        try:
            ...
            if self.pipewire_enabled == False and output_device_string != self.llm_tts_output_device_string:
                ...
                self.start_llm_tts(True) # force restart
            ...
            try:
                if voice_messages_queue.empty() == False:
                    ...
                    if next_message != None:
                        if self.DEBUG:
                            print("speak_thread: handling next voice_message in the queue: " + str(next_message))
                        ...
                        if 'voice_message' in next_message:
                            ...
                            if str(next_message['voice_message']) != self.main_controller_missing_warning:
                                if self.DEBUG:
                                    print("voice_thread: just spoke")
                                just_spoke = True
                    ...
                    if self.DEBUG:
                        print("speak_thread: just spoke was true, but voice_message_queue is now empty again\n\n🤭 DONE TALKING\n\n")
                    just_spoke = False
                    
                    #if self.llm_assistant_continue_conversation:
                    #    if self.DEBUG:
                    #        print("self.llm_assistant_continue_conversation was True")
                    #    self.llm_assistant_continue_conversation -= 1 
                    #    self.send_wakeword_detected_message(None,{'origin':'voice','llm_assistant_conversation':True})
            ...
            try:
                ...
                if self.llm_assistant_continue_conversations and self.llm_tts_started and self.llm_tts_process != None and self.llm_tts_process.poll() == None and self.llm_tts_process.stdout != None:
                    try:
                        ...
                        tts_stdout = str(os.read(self.llm_tts_process.stdout.fileno(), 10240))
                        if self.DEBUG:
                            print("speak_thread: tts_stdout: " + str(tts_stdout))
                        self.llm_tts_process.stdout.flush()
                        
                        if '(infer=' in str(tts_stdout):
                            tts_stdout_last_part = str(tts_stdout).split('(infer=')[-1]
                            if self.DEBUG:
                                print("\nspeak_thread: tts_stdout_last_part:\n" + str(tts_stdout_last_part) + "\n")
                                
                            numbers = re.findall(r"[-+]?\d*\.\d+|\d+", tts_stdout_last_part)
                            if self.DEBUG:
                                print("speak_thread: timing numbers from tts process: " + str(numbers))
                            speaking_time = 0
                            if len(numbers) > 1:
                                speaking_time += float(numbers[1])
                            #for number in numbers:
                            #    print("number: " + str(number))
                            #    speaking_time += float(number)
                            if self.DEBUG:
                                print("speak_thread: total speaking_time: " + str(speaking_time))
                            if speaking_time > 0:
                                self.llm_tts_waiting_for_speaking_to_finish = True
                ...
                if self.DEBUG:
                    print("speak_thread: Caught error getting STDOUT from LLM TTS proces: " + str(ex))

            if self.llm_tts_waiting_for_speaking_to_finish and time.time() < self.llm_tts_really_done_speaking_time:
                if self.DEBUG:
                    print("Waiting for speaking to finish: " + str(self.llm_tts_really_done_speaking_time - time.time()))
                
            elif self.llm_tts_waiting_for_speaking_to_finish and time.time() > self.llm_tts_really_done_speaking_time:
                if self.DEBUG:
                    print("\n\n🤭 REALLY DONE SPEAKING NOW\n\n")
                self.llm_tts_waiting_for_speaking_to_finish = False
                if self.llm_assistant_continue_conversation == 2:
                    #self.llm_assistant_continue_conversation -= 1
                    if self.DEBUG:
                        print("speak_thread: REALLY DONE SPEAKING -> in a conversation -> calling send_wakeword_detected_message")
                    self.send_wakeword_detected_message()
                elif self.llm_assistant_continue_conversation == 0:
                    if self.periodic_voco_attempts > 2:
                        if self.DEBUG:
                            print("speak_thread: not in a conversation, and the main controller is missing. Will attempt to speak warning about missing main controller.")
            ...
            if self.persistent_data['site_id'] == str(site_id):
                ...
                if 'customData' in intent and \
                  ...
                  str(intent['customData']['parsed_by']) == self.persistent_data['main_site_id']:
                    if self.DEBUG:
                        print("- really_speak: debug: this message has been parsed by the main controller")
                    self.voco_connected = True
                    self.periodic_voco_attempts = 0
...
#
        ...
        try:
            ...
            for x in range(py_audio.get_host_api_count(),-1,-1):
                ...
                try:
                    ...
                    if self.DEBUG:
                        print("run_wakeword: info of device at index x: " + str(info))
    ...
    def send_wakeword_detected_message(self, wakeword=None,custom_entities=None):
        if wakeword == None:
            wakeword = str(self.persistent_data['llm_wakeword_model'])
        if self.DEBUG:
            ...
            if time.time() > self.current_utc_time + 1:
                ...
                if self.signal_started:
                    ...
                    if self.signal_counter > 5:
                        self.signal_counter = 0
                        if self.DEBUG:
                            print("Clock: getting Signal messages")
                ...
                try:
                    ...
                    last_controller_ping_seconds_ago = 0
                    if self.main_controller_last_ping_received_time != 0:
                        last_controller_ping_seconds_ago = int(time.time()) - int(self.main_controller_last_ping_received_time)
                    ...
                    if time.time() - self.slow_loop_interval > self.last_slow_loop_time: # + self.minimum_injection_interval > datetime.utcnow().timestamp():
                        ...
                        if self.DEBUG:
                            ...
                            if self.main_controller_last_ping_received_time > 0:
                                print("   main controller ping received seconds ago: ", int(time.time()) - self.main_controller_last_ping_received_time)
                            if self.last_pong_time > 0:
                                print("   pong seconds ago: ", int(time.time()) - self.last_pong_time)
                        ...
                        if time.time() > self.llm_last_assistant_reponse_time + 120 and time.time() > self.llm_last_assistant_query_time + 120:
                            self.llm_chat_history = []
                        ...
                        if self.persistent_data['is_satellite'] and self.persistent_data['main_site_id'] == self.persistent_data['site_id']:
                            if self.DEBUG:
                                ...
                                print(" - self.persistent_data['site_id']     : ", self.persistent_data['site_id'])
                                print(" - self.persistent_data['main_site_id']: ", self.persistent_data['main_site_id'])
                        ...
                        try:
                            ...
                            if self.still_busy_booting == True and time.time() - self.addon_start_time > 120:
                                if self.DEBUG:
                                    print("\nWARNING, Voco addon started 120 seconds ago, and is still busy booting...\n")
                                    print(" - self.still_busy_booting?: ", self.still_busy_booting)
                                    print(" - self.initial_injection_completed?: ", self.initial_injection_completed)
                                self.still_busy_booting = False
                            ...
                            if self.mqtt_client != None:
                                ...
                                if self.still_busy_booting == False:
                                    ...
                                    if self.should_restart_snips == True:
                                        ...
                                        if self.persistent_data['is_satellite'] and self.this_is_main_controller == False and self.mqtt_connected:
                                            self.periodic_voco_attempts += 1
                                        ...
                                        if self.mqtt_connected == False and self.periodic_mqtt_attempts%5 == 4:
                                            if self.DEBUG:
                                                print("clock: Should attempt to find correct MQTT server IP address.  self.periodic_mqtt_attempts: ", self.periodic_mqtt_attempts)
                                        ...
                                        if self.periodic_voco_attempts > 5:
                                            if self.DEBUG:
                                                print("clock: main Voco controller has not responded in a while. It may be down permanently.")
                                            self.voco_connected = False
                                    ...
                                    if self.DEBUG:
                                        print("\nWARNING, clock: still busy booting!! self.mqtt_connected_succesfully_at_least_once?: " + str(self.mqtt_connected_succesfully_at_least_once))
                            ...
                            if (self.current_utc_time >= int(item['moment']) and self.current_utc_time < int(item['moment']) + 60) or item['type'] == 'countdown':
                                if self.DEBUG:
                                    if item['type'] != 'countdown':
                                        ...
                                        print("clock: non-countdown timer item:\n" + str(json.dumps(item,indent=4)))
                    ...
                    if last_controller_ping_seconds_ago > 999999900:
                        if self.DEBUG:
                            print("resetting self.main_controller_last_ping_received_time as it was getting pretty big")
                        self.main_controller_last_ping_received_time = int(time.time())
...
#
    ...
    # Check if a controller has recently pinged this controller
    def controller_recently_spotted(self,site_id):
        if self.DEBUG2:
            print("in controller_recently_spotted.  site_id: " + str(site_id))
        for controller in self.mqtt_others:
            try:
                #if self.DEBUG:
                #    print("controller in self.mqtt_others: " + str(json.dumps(self.mqtt_others[controller],indent=4)))
                if 'siteId' in self.mqtt_others[controller] and str(self.mqtt_others[controller]['siteId']) == str(site_id):
                    #print("controller_recently_spotted: so far so good")
                    if 'last_seen' in self.mqtt_others[controller] and int(self.mqtt_others[controller]['last_seen']) > time.time() - 60:
                        if self.DEBUG2:
                            print("controller_recently_spotted: YES")
                        return True
            except Exception as ex:
                if self.DEBUG2:
                    print("Caught ERROR checking if controller was recently spotted: " + str(ex))
        if self.DEBUG2:
            print("controller_recently_spotted: NO")
        return False
...
#
    ...
    def on_second_disconnect(self, client, userdata, rc):
        ...
        if rc == 0:
            ...
            if rc == 7:
                if self.DEBUG:
                    print("\nVoco is likely running twice\nAttempting to self-terminate (blocked)")
    ...
    def on_second_message(self, client, userdata, msg):
        ...
        if msg.topic == 'hermes/asr/textCaptured':
            if self.DEBUG:
                print("second client: received message to hermes/asr/textCaptured")
            ...
            if self.persistent_data['is_satellite']:
                ...
                if self.periodic_voco_attempts > 2:
                    if self.DEBUG:
                        print("/textCaptured: satellite: NOT PUBLISHING TO MAIN CONTROLLER - IT SEEMS TO BE DOWN")
                    pass
                    #self.speak(self.main_controller_missing_warning)
                else:
                    ...
                    if self.persistent_data['listening']:
                        ...
                        if 'unknownword' in str(payload['text']) or str(payload['text']) == '':
                            ...
                            # TODO: currently not immediately sending command to main controller. This might be a bad idea.
                            if self.DEBUG:
                                print("textCaptured: satellite, so sending captured text to main controller (DISABLED!): " + str(payload['text']))
        ...
        if msg.topic.startswith('hermes/nlu/intentParsed'):
            ...
            if 'id' in payload and payload['id'].endswith('fafafafa'): # self.llm_stt_always_use and
                ...
                # TODO: this might be a bad idea. But having no site ID is even worse.
                if not 'siteId' in intent_message:
                    if self.DEBUG:
                        print("hermes/nlu/intentParsed: intent didn't have siteId. Setting it to mine.")
                    intent_message['siteId'] = self.persistent_data['site_id']
            ...
            if 'id' in payload and payload['id'].endswith('fafafafa'): # self.llm_stt_always_use and
                ...
                if self.DEBUG:
                    print("\nNOTE: intentNotRecognized: this may be an opportunity to send the text to the assistant.")
                    print("- payload: " + str(payload))
                ...
                """
                    ...
                    if self.DEBUG:
                        ...
                        print(" - self.llm_stt_sentence: " + str(self.llm_stt_sentence))
            ...
            try:
                ...
                print("on_second_message: error handling message to topic hermes/intent: " + str(ex))
        ...
        elif msg.topic.startswith('hermes/injection/perform'):
        ...
        # TODO: what happens if the main controller and the satellite both hear the command in this new situation?
            ...
            if self.DEBUG:
                print("      -> 2nd MQTT MESSAGE FELL THROUGH. msg.topic: \n" + str(msg.topic))
    ...
    def run_mqtt(self):
        ...
        if self.last_run_mqtt_time > int(time.time()) - 10:
            if self.DEBUG:
        ...
        if self.persistent_data['is_satellite'] and str(self.persistent_data['mqtt_server']) == self.ip_address:
            if self.DEBUG:
                print("\nError, the MQTT server IP address was the device's own IP address. Because this is a satellite, this shouldn't be the case.")
            ...
            if self.DEBUG:
                print("\nError, not a satellite, but self.persistent_data['mqtt_server'] was not 127.0.0.1 / 0.0.0.0: ", self.persistent_data['mqtt_server'])
                print("Setting it to self.internal_ip: ", self.internal_ip)
            self.persistent_data['mqtt_server'] = self.internal_ip
        ...
        try:
            if self.mqtt_client != None:
                if self.DEBUG:
                    print("run_mqtt: MQTT Client already existed. Not stopping and restarting it, it will keep trying by itself.")
                ...
                if self.should_restart_mqtt:
                    ...
                    if self.mqtt_client.is_connected():
                        if self.DEBUG:
                            print("run_mqtt: MQTT client already exists and says it is already connected to the correct server. Aborting run_mqtt")
                        return
            ...
            if self.mqtt_client == None:
                ...
                if self.persistent_data['is_satellite']:
                    if str(self.persistent_data['mqtt_server']) == self.ip_address:
                        if self.DEBUG:
                            print("\nERROR: run_mqtt: the MQTT server IP address was the device's own IP address. Because this is a satellite, this shouldn't be the case. Continuing anyway.")
            ...
            """
                ...
                if self.mqtt_connected == False and self.mqtt_busy_connecting == False:
                    ...
                    if self.persistent_data['is_satellite']:
                        ...
                        if self.DEBUG:
                            print("run_mqtt: This device is NOT a satellite, so MQTT client is (re)connecting to internal_ip and port: " + str(self.internal_ip) + ":" + str(self.mqtt_port))
                        self.mqtt_connected_to_ip = str(self.internal_ip)
                        self.mqtt_client.connect(str(self.internal_ip), int(self.mqtt_port), keepalive=60)
                else:
                    if self.DEBUG:
            ...
            #self.mqtt_client.loop_forever() # TODO: this was inside the following if statement:
            if self.should_restart_mqtt:
                # self.mqtt_client.loop_start()
                self.should_restart_mqtt = False
                if self.DEBUG:
                    print("run_mqtt: MQTT client loop (re)started. set self.should_restart_mqtt back to false")  
            

            # self.mqtt_client.tls_set(certfile='/home/pi/.webthings/ssl/certificate.crt') ?
            #self.mqtt_client.tls_set(
            #    ca_certs='./server-ca.crt',
            #    certfile='./client.crt', # /home/pi/.webthings/ssl/certificate.crt ?
            #    keyfile='./client.key'
            #)
            ...
            if self.DEBUG:
                print("run_mqtt: caught error creating MQTT client connection: " + str(ex))
            self.mqtt_connected = False
            self.voco_connected = False
            self.mqtt_busy_connecting = False
            ...
            if '111' in str(ex): # [Errno 111] Connection refused
                ...
                if self.DEBUG:
                    print("- Error 113 - failed to connect to main voco controller (No route to host)")
                    print("- - self.currently_scanning_for_missing_mqtt_server: " + str(self.currently_scanning_for_missing_mqtt_server))
            ...
            time.sleep(1)
            if self.DEBUG:
                print("run_mqtt error -> setting should_restart_mqtt to True")
            self.should_restart_mqtt = True
    ...
    def on_disconnect(self, client, userdata, rc):
        ...
        #self.voco_connected = False
        if self.mqtt_connected:
            self.mqtt_last_disconnect_timestamp = int(time.time())
        self.mqtt_connected = False
        self.voco_connected = False
        self.mqtt_busy_connecting = False
        self.should_restart_mqtt = True
        ...
        if rc == 0:
            ...
            if self.DEBUG:
                ...
                print("on_disconnect: self.mqtt_connected: " + str(self.mqtt_connected))
            #self.mqtt_connected = False
            if rc == 7:
    ...
    def on_connect(self, client, userdata, flags, rc): # rc = 'reason_code'
        if self.DEBUG:
            print("In on_connect (mqtt). rc: " + str(rc))
        self.mqtt_connected_succesfully_at_least_once = True
        self.mqtt_busy_connecting = False
        self.should_restart_mqtt = False
        ...
        if rc == 0:
            ...
            try:
                ...
                # TODO: experiment with moving this to the satellite too
            ...
            #self.last_on_second_disconnect_time = int(time.time())
    ...
    # self._userdata, mid, granted_qos
    def on_subscribe(self, client, userdata, rc, whatever):
        if self.DEBUG:
            print("in on_subscribe")
            print("on_subscribe: client, userdata, rc, whatever: ", client, userdata, rc, whatever)

    # "The callback called when the broker responds to an unsubscribe request"
    def on_unsubscribe(self, client, userdata, rc, whatever):
        if self.DEBUG:
            print("in on_unsubscribe")
            print("on_unsubscribe: client, userdata, rc, whatever: ", client, userdata, rc, whatever)
    ...
    def on_message(self, client, userdata, msg):
        ...
        #    self.parse_ping(payload,ping_type="ping")
            ...
            # broadcast ping messages are now sent continously
        ...
        if msg.topic.startswith('hermes/voco/ping'):
            ...
            if self.DEBUG:
                print("received pong at topic: ", msg.topic)

            if 'siteId' in payload:
                if payload['siteId'] == self.persistent_data['site_id']:
                    if self.DEBUG:
                        print("spotted the pong that I sent") # What we want is to receive a pong from the main controller
                elif self.persistent_data['is_satellite'] == True and self.persistent_data['main_site_id'] != self.persistent_data['site_id'] and payload['siteId'] == self.persistent_data['main_site_id']:
                    if self.DEBUG:
                        print("spotted a pong sent by the main controller")
                    self.voco_connected = True
                    self.periodic_voco_attempts = 0
                    self.parse_ping(payload,ping_type="pong")
                else:
        ...
        elif msg.topic == 'hermes/voco/parse':
            if self.DEBUG:
        ...
        # this is used to catch when a session has been started to parse text input
            ...
            try:
                ...
                if 'origin' in intent_message and intent_message['origin'] == 'voice' and self.persistent_data['listening'] == False and intent_message['siteId'].endswith(self.persistent_data['site_id']):
                    ...
                    # TODO: doesn't this defeat the purpose of avoiding doing things twice when two controllers both hear the same command?
                    if not intent_message['siteId'].endswith(self.persistent_data['site_id']):
                        self.previous_intent_callback_time = time.time()
                ...
                rint("Error handling incoming Snips MQTT message: " + str(ex))  
        ...
        # deprecated
        elif msg.topic.startswith("hermes/voco/gettime"):
            if not self.persistent_data['is_satellite']:
                if self.DEBUG:
        ...
        if msg.topic.startswith("hermes/voco/" + self.persistent_data['site_id']):
            ...
            try:
                ...
                #    self.parse_ping(payload,ping_type="pong")
                    ...
                    if self.DEBUG:
                        print("Received /parse request specifically for this controller.  text: " + str(payload['text']))
    ...
    def send_mqtt_ping(self, broadcast=False, ping_type="ping", target_site_id=None):
        ...
        if self.mqtt_connected and self.ip_address != None:
            try:
                ...
                #if ping_type == 'ping' and broadcast == False:
                #    self.ping_counter += 1
                #    if self.ping_counter > 10:
                #        if self.DEBUG:
                #            print("turning a normal ping into a broadcast ping after 10 normal pings were sent")
                #        self.ping_counter = 0

                mqtt_ping_path = "hermes/voco/ping"
                ...
                if self.DEBUG:
                    print("send_mqtt_ping:  mqtt_ping_path: ", mqtt_ping_path)

                if str(ping_type) == 'pong':
                    self.last_pong_time = int(time.time())
                else:
                    self.last_ping_time = int(time.time())

                if isinstance(mqtt_ping_path,str) and isinstance(self.hostname,str) and isinstance(self.ip_address,str):
                    ...
                    if self.DEBUG:
                        print("ERROR: send_mqtt_ping:  mqtt_ping_path or self.hostname not a string: ", self.hostname)
                ...
                if self.DEBUG:
                    print("caught error in send_mqtt_ping: " + str(ex))
        else:
            if self.DEBUG:
                print("Warning, not sending broadcast ping. self.mqtt_connected was likely false: ", self.mqtt_connected)
    ...
    def update_fastest_controller(self):
        ...
        if fastest_id == self.persistent_data['site_id']:
            fastest_last_ping_time = int(time.time())
        ...
        if 'ip' in payload and 'siteId' in payload and 'hostname' in payload and 'satellite' in payload and 'main_controller' in payload and 'thing_titles' in payload:
            ...
            if payload['siteId'] != self.persistent_data['site_id']:
                ...
                if ('running' in payload and payload['running'] == False) or ('ping_type' in payload and payload['ping_type'] == 'goodbye'):
                    ...
                    self.mqtt_others[payload['ip']]['last_bye_timestamp'] = int(time.time())
                    #del self.mqtt_others[payload['ip']]

                    self.update_fastest_controller()

                    if self.persistent_data['is_satellite'] and self.persistent_data['main_site_id'] != self.persistent_data['site_id'] and str(payload['siteId']) == self.persistent_data['main_site_id']:
                        if self.DEBUG:
                            print("\nTHE MAIN CONTROLLER SAID GOODBYE\n")
                        self.main_controller_goodbye_timestamp = int(time.time())
                ...
                if self.persistent_data['is_satellite']:
                    ...
                    if payload['ip'] != self.ip_address and payload['ip'] == self.persistent_data['mqtt_server'] and self.persistent_data['main_site_id'] != self.persistent_data['site_id'] and str(payload['siteId']) == self.persistent_data['main_site_id']:
                        if self.persistent_data['main_controller_ip'] != payload['ip']:
                            self.persistent_data['main_controller_ip'] = payload['ip']
                            self.save_to_persistent_data = True
                            if self.DEBUG:
                                print("parse_ping: satellite received a new IP address for the main controller. Saving it: ", payload['ip'])

                    elif payload['ip'] != self.ip_address and payload['hostname'] == self.persistent_data['main_controller_hostname']: # and self.persistent_data['main_site_id'] == self.persistent_data['site_id']: # and self.persistent_data['main_site_id'] == self.persistent_data['site_id']:
                        if self.DEBUG:
                        ...
                        if self.persistent_data['main_site_id'] != payload['siteId']:
                            ...
                            self.save_to_persistent_data = True
                            if self.DEBUG:
                                print("parse_ping: satellite received the site_id of the main controller. Saving it: ", payload['siteId'])
                ...
                if payload['siteId'] == self.persistent_data['main_site_id']: # and self.persistent_data['main_site_id'] != self.persistent_data['site_id']:
                    if self.DEBUG:
                        print("parse_ping: good response from main controller.  payload: ", payload)
                    ...
                    if self.persistent_data['is_satellite'] == True and self.persistent_data['main_site_id'] != self.persistent_data['site_id']:
                        ...
                        if self.persistent_data['mqtt_server'] != payload['ip'] and payload['ip'] != '127.0.0.1' and payload['ip'] != '0.0.0.0' and payload['ip'] != 'localhost':
                        ...
                        if self.currently_scanning_for_missing_mqtt_server and payload['ip'] != '127.0.0.1' and payload['ip'] != '0.0.0.0' and payload['ip'] != 'localhost':
                            if self.DEBUG:
                                print("Received ping from missing controller while currently_scanning_for_missing_mqtt_server was true. Aborting scan.")
                            self.currently_scanning_for_missing_mqtt_server = False
    ...
    def query_intent(self, sentence='', intent=None, session_id=None):
        ...
        try:
            ...
            if len(sentence) > 1:
                ...
                query_id = 'b3faa0ff-39e8-' + str(randomHex(4)) + '-' + str(randomHex(4)) + '-' + str(randomHex(4)) + 'fafafafa' #8e0e
...
#
    ...
    # Triggered everytime Snips succesfully recognizes a voice intent,
    # and is also fed from other sources, like hermes/voco/parse and query_intent()
    def master_intent_callback(self, intent_message=None, try_alternative=False):    
        if self.DEBUG:
        ...
        try:
            ...
            if word_count == 1:
                ...
                if (self.llm_stt_started or (self.fastest_controller_id != None and time.time() < self.fastest_controller_last_ping_time + 60)) and 'id' in intent_message and not intent_message['id'].endswith('fafafafa'):
                    if self.DEBUG:
                    ...
                    if self.DEBUG:
                        print("STT not a possibility, aborting.")
        ...
        self.is_this_main_controller() # also updates self.this_is_main_controller
            
        if all_possible_intents_count == 0:
            # TODO: doesn't this cause a return earlier in the code? Superfluous?
            self.try_again_via_assistant = True
        ...
        for incoming_intent, slots in all_possible_intents_data.items():
            ...
            try:
                ...
                if slots['start_time'] != None:
                    if slots['start_time'] < time.time():
                        if self.DEBUG:
                            print("Error, start time in slots was before the current time. Setting start_time slot to None")
            ...
            try:
                ...
                if incoming_intent == 'get_time':
                    if self.DEBUG:
                        print("incoming intent is get_time")
                ...
                if incoming_intent == 'get_value' or incoming_intent == 'set_value' or incoming_intent == 'set_state' or incoming_intent == 'get_boolean':
                                        ...
                                        for satellite_thing_title in self.persistent_data['satellite_thing_titles'][satellite_id]:
                                            if target_thing_title != None:
                                                if satellite_thing_title.lower() == target_thing_title.lower():
                                                    if self.DEBUG:
                                                        print("A satellite has this thing (" + str(target_thing_title) + "), it should (also) handle it: " + str(satellite_id))
                    ...
                    if found_on_satellite or (found_on_satellite == False and len(satellites_with_the_thing) > 0): #and (self.persistent_data['is_satellite'] or self.this_is_main_controller == True): # TODO: are these secondary checks necessary?
                        ...
                        if self.persistent_data['listening'] == False and 'origin' in intent_message and intent_message['origin'] == 'voice':
                            if self.DEBUG:
                                 print("not passing voice command to main/other controller as listening is disabled")
                            return
       ...
       #     
        ...
        try:
            ...
            if self.persistent_data['is_satellite'] and this_is_origin_site == False and found_thing_on_satellite == False and voice_message.startswith('Sorry'):
                ...
                # TODO: maybe satellites that handle things should be restricted to exact uniquely named things or perfect things+property matches? # DONE
                # Maybe the satellites could also track which devices are on the main controller, and not do a thing scan if the desired title was in the list of things on the main controller. # DONE
                ...
                if (best_confidence_score != 1 
                    ...
                    if voice_message.startswith('Sorry'):
                        if self.DEBUG:
                            self.speak("Debug, almost spoke: " + str(voice_message), intent=intent_message)
                    else:
                        self.speak(voice_message,intent=intent_message)
    ...
    # This broadcasts to delete this action item over MQTT to all Voco devices. All of the controllers, including the originating one, then run remove_action_time when they receive that message
    def broadcast_remove_action_time(self,delayed_action):
        ...
        self.remove_action_time(delayed_action) # doesn't this cause it to run twice? No harm though.
        return True
    ...
    def remove_action_time(self,delayed_action): # returns True of False, which is useful for the API handler, which calls this
        ...
        if moment != None and sentence != None:
            ...
            if item_to_delete != None:
                ...
                if self.DEBUG:
                    print("Error, remove_action_time: could not find element to delete in action times list")
        else:
            if self.DEBUG:
                print("Error: remove_action_time: moment and/or sentence was missing from action item, so cannot detete it")
    ...
    def inject_updated_things_into_snips(self, force_injection=False):
        ...
        try:
            ...
            if self.try_updating_things():
                ...
                #if self.persistent_data['is_satellite'] == False:
                # TODO: actually, satellite should be able to understand titles on main controller as well? Maybe?
                
                # keep a copy of the local-only thing titles list
                local_thing_titles_list = full_thing_titles_list.copy()

                if self.DEBUG:
                    print("inject_updated_things_into_snips: fresh_property_titles count: ", len(fresh_property_titles))
                    print("inject_updated_things_into_snips: injection_stopwatch delta: ", time.time() - injection_stopwatch_start)
                ...
                #    print("fresh_prop_titles = " + str(fresh_property_titles))
                    print("fresh_prop_strings length: ", len(fresh_property_strings))
                ...
                if len(thing_titles^fresh_thing_titles) > 0 or self.force_injection == True: # comparing sets to detect changes in thing titles
                    ...
                    #if not self.persistent_data['is_satellite']:
                    for multi_thing in self.multi_things:
                        fresh_thing_titles.add('multi_thing')
                    #fresh_thing_titles.add('lights')
                    #fresh_thing_titles.add('curtains') # TODO: not used yet
...
#
    ...
    def check_things(self, intent, slots):
            ...
            try:
                if result != None:
                    ...
                    if thing_must_have_capability == None and property_must_have_capability == None:
                        ...
                        if boolean_related and slots['thing'] != None and slots['thing'] not in self.multi_things and slots['property'] == None: # TODO: and slots['thing'] != 'curtains'
                            if self.DEBUG:
                                print("A singular thing was desired. Pruning back to the most likely property")
        ...
        # TODO: there seems to be some code duplication here
        ...
        if len(result) > 1 and only_allow_one_thing_scanner_result: # one specific property was intended, so only one result is allowed.
            ...
            # TODO: doesn't the thing scanner already do this at the end? See a few lines above..
            if best_matched_found_property != None:
                if self.DEBUG:
                ...
                if self.DEBUG:
                    print("- THERE WERE RESULTS, BUT NOT RETURNING ANY") # TODO: why return none? The thing scanner above selects the first item on the list.
                result = []
    ...
    # Note: this gets fed from both hermes/voco/parse (broadcast) and hermes/voco/[site_id]/parse (targetted to this controller), and from UI text chat commands
    # Currently the broadcast route doesn't seem to be used, with satellites surgically passing along intent messages to only the relevant controllers (that have the targetted things)
    # (Before, all satellites would receive all intents, and just stay quiet if they couldn't do anything with it).
    def parse_text(self, origin=None, site_id=None):
        ...
        if 'unknownword' in self.last_text_command:
            if self.DEBUG:
                print("Error, got message to parse_text, but the message contained 'unknownword'")
            self.last_text_command = ''
        ...
        if site_id == str(self.persistent_data['site_id']):
            ...
            if self.persistent_data['is_satellite'] == True and self.mqtt_connected and self.periodic_voco_attempts < 3:
                ...
                if origin == 'text' or origin == 'matrix' or origin == 'llm_stt':
                    if self.DEBUG:
                        print("parse_text: satellite, and connected to main controller, so sending start_session there)")
                    ...
                    if self.DEBUG:
                        print("voice origin, so not doing startSession on main controller")
            ...
            if self.persistent_data['is_satellite'] == False:
                ...
                if self.DEBUG:
                    print("parse_text: handling parse_text with a remote site_id. I am a satellite. Doing nothing for now.")
        else:
            if self.DEBUG:
                print("\nERROR, parse_text: not connected to second MQTT broker?\n")
    ...
    def look_for_mqtt_server(self):
        try:
            ...
            if self.mqtt_last_disconnect_timestamp == 0:
                if self.DEBUG:
                    print("WARNING: look_for_mqtt_server: self.mqtt_last_disconnect_timestamp was still zero. Setting it to now_stamp")
                self.mqtt_last_disconnect_timestamp = int(time.time())
                return False

            if self.mqtt_last_disconnect_timestamp > int(time.time()) - 300:
                if self.DEBUG:
                    print("look_for_mqtt_server: mqtt_last_disconnect_timestamp was less than 5 minutes ago.. aborting search for MQTT server, since the odds are that the other controller is just rebooting and reconnection will occur naturally soon")
                return False
            ...
            if self.persistent_data['is_satellite']:
                ...
                for controller_ip in possible_controllers:
                    if self.DEBUG:
                        print("look_for_mqtt_server: possible_controllers loop: controller_ip: " + str(controller_ip))
                    if controller_ip != '127.0.0.1' and controller_ip != '0.0.0.0' and controller_ip != 'localhost':
                ...
                if self.DEBUG:
                    print("look_for_mqtt_server: not a satellite, so not starting a search for a main controller")
                return False
    ...
    def is_this_main_controller(self):
        ...
        for sat in self.connected_satellites:
            if self.DEBUG:
                print("is_this_main_controller: checking if recently connected: " + str(sat))
        ...
        if self.DEBUG:
            print(" - acting_as_main_controller: " + str(acting_as_main_controller))
        self.this_is_main_controller = acting_as_main_controller
            ...
            for key in self.llm_models.keys():
                ...
                for model_name in self.llm_models[key]['list']:
                    ...
                    if str(self.persistent_data[persistent_key]) == 'voco':
                        ...
                        if self.llm_models[key]['list'][model_name]['model'] == str(self.persistent_data[persistent_key]):
    ...
    def start_llm_servers(self):
        ...
        if self.llm_enabled == False:
            if self.DEBUG:
                print("start_llm_servers thread: ABORTING, LLM is fully disabled")
            return
        ...
        if self.llm_tts_enabled:
            if self.DEBUG:
                print("start_llm_servers thread: calling start_llm_tts()")
            self.start_llm_tts()
    ...
    def start_llm_tts(self, restart=False):
        ...
        if self.bits < 64:
            if self.DEBUG2:
                print("cannot start LLM TTS on 32 bit system")
    ...
    def try_llm_stt(self,intent=None):
        if self.DEBUG:
            print("\nin try_llm_stt. intent: " + str(intent))

        if (self.llm_enabled and self.llm_stt_enabled and self.llm_stt_started) or self.fastest_controller_id != None and self.fastest_controller_last_ping_time > time.time() - 60: # and (self.try_again_via_stt or self.try_again_via_assistant):
            if self.DEBUG:
                print("- try_llm_stt. calling llm_stt. Intent: " + str(intent))
    ...
    def llm_stt(self,intent=None):
        ...
        # Opportunity to quickly restart the STT server?
        #if self.llm_stt_started == False:
        #

        if self.llm_enabled and self.llm_stt_enabled and self.llm_stt_started and self.fastest_controller_id == self.persistent_data['site_id']:
            try:
        ...
        elif self.mqtt_client != None and self.mqtt_connected and self.fastest_controller_id != self.persistent_data['site_id'] and self.fastest_controller_last_ping_time + 60 > time.time():
            if self.DEBUG:
            ...
            if self.DEBUG:
                print("WARNING, STT SERVER HAS NOT STARTED (and other controller is not an option either)")
                print(" - self.llm_stt_started: " + str(self.llm_stt_started))
        ...
        # TODO: these might be too stringent checks:
        """
    ...
    def parse_llm_stt_result(self, stt_result=None, intent=None):
        ...
        if stt_result == None or stt_result == 'None':
            ...
            self.speak("Sorry, speech recognition failed",intent=intent)
            return
    ...
    def start_llm_stt_server(self):
        ...
        if self.llm_enabled and self.llm_stt_enabled:
            if self.free_memory > self.llm_stt_minimal_memory:
                ...
                # host was '0.0.0.0',
                # Hmm, 127.0.0.1 also wasn't working. Back to 0.0.0.0...
    ...
    def ask_a_question(self):
        ...
        self.llm_last_assistant_query_time = time.time()
        self.play_sound()
    ...
    def stop_ai_assistant(self):
        ...
        try:
            if self.llm_assistant_process != None and self.llm_assistant_process.poll() == None:
                ...
                if self.DEBUG:
                    print("stop_ai_assistant: process already stopped?")
            ...
            if self.llm_enabled and self.llm_assistant_enabled:
                if self.free_memory > self.llm_assistant_minimal_memory:
                    ...
                    if str(self.persistent_data['llm_assistant_model']) == 'ggml-model-i2_s.gguf': #.lower().startswith('bitnet'):
                        ...
                        model_path = os.path.join(self.llm_assistant_data_dir_path,self.persistent_data['llm_assistant_model'])
                        ...
                        assistant_shell_command = "LD_LIBRARY_PATH=" + str(self.llm_assistant_addon_dir_path ) + " " + ' '.join(assistant_command)
                        if self.DEBUG:
                            print("\n--- - - +\nbitnet assistant_command: ", assistant_command)
                            print("a.k.a.")
                            print(assistant_shell_command)
                            print("")

                        bitnet_environment = os.environ.copy()
                        #environment["LD_LIBRARY_PATH"] = '{}:{}'.format(self.snips_path,self.arm_libs_path)
                        bitnet_environment["LD_LIBRARY_PATH"] = str(self.llm_assistant_addon_dir_path )
                        
                        #assistant_command.insert(0, 'exec')

                        #ld_path_prefix = "LD_LIBRARY_PATH=" + str(self.llm_assistant_addon_dir_path )
                        #assistant_command.insert(0, ld_path_prefix)
    ...
    def check_if_message_is_for_actuators(self, message):
        ...
        # self.llm_assistant_first_words_to_avoid = ['disable','stop','enable','start','create','set']
        for first_word in self.llm_assistant_first_words_to_avoid:
    ...
    def ask_ai_assistant(self,voice_message=None,intent=None):
        ...
        try:
            ...
            if self.mqtt_client != None and self.mqtt_connected and self.fastest_controller_id != self.persistent_data['site_id'] and self.fastest_controller_last_ping_time + 60 > time.time():
                ...
                if self.llm_assistant_process.poll() != None:
                    if self.DEBUG:
                        print("\nERROR: ask_ai_assistant: self.llm_assistant_process has died? Setting self.restart_llm_servers to True")
                    self.restart_llm_servers = True
                    time.sleep(2)
                if self.DEBUG:
                ...
                if self.DEBUG:
                    ...
                    self.speak("debug: assistant dead end", intent)
        except Exception as ex:
            if self.DEBUG:
    ...
    def really_ask_ai_assistant(self,voice_message,intent=None):
        ...
        voice_message = voice_message.lstrip()
        voice_message = voice_message.rstrip()
        original_voice_message = voice_message
        lowercase_voice_message = voice_message.lower()
        ...
        if self.check_if_message_is_for_assistant(voice_message):
            ...
            if not voice_message.endswith('?') and (
                                lowercase_voice_message.startswith('who ') 
                                or lowercase_voice_message.startswith('what ') 
                                or lowercase_voice_message.startswith('where ') 
                                or lowercase_voice_message.startswith('how ') 
                                or lowercase_voice_message.startswith('what ') 
                                or lowercase_voice_message.startswith('can ')
                                ):
            ...
            if ' show me ' in display_test or display_test.startswith('show ') or display_test.startswith('show us ') or display_test.startswith('please show ') or display_test.startswith('can you show ') or display_test.startswith('display ') or display_test.startswith('please display ') or display_test.startswith('can you display ') or display_test.startswith('can you please display '):
                ...
                display = True
                if origin == 'voice':
                    old_intent = intent.copy()
                    self.speak('OK, I will show you on a display',old_intent)
                origin = 'text'
                intent['origin'] = 'text'
            ...
            if str(self.persistent_data['llm_assistant_protocol']) not in self.llm_assistant_protocols:
                if self.DEBUG:
                    print("ERROR, that protocol is not available. Setting to basic instead.")
                self.persistent_data['llm_assistant_protocol'] = 'basic'
                self.save_persistent_data()

            if isinstance(self.persistent_data['llm_assistant_protocol'], str) and self.persistent_data['llm_assistant_protocol'] in self.llm_assistant_protocols:
                self.llm_assistant_protocol = self.llm_assistant_protocols[ str(self.persistent_data['llm_assistant_protocol']) ]
            else:
                self.llm_assistant_protocol = self.llm_assistant_protocols[ 'basic' ] #'basic'
            if self.DEBUG:
                print("really_ask_ai_assistant:  self.llm_assistant_protocol: " + str(self.llm_assistant_protocol))

            if self.llm_assistant_protocol != None and 'user' in self.llm_assistant_protocol:
                voice_message = self.llm_assistant_protocol['user'].replace('{user_message}', voice_message)
            ...
            if self.llm_assistant_process == None:
                if self.DEBUG:
                    print("self.llm_assistant_process is still None")
            ...
            if self.llm_assistant_process != None: # and self.llm_assistant_process.poll() == None:
                ...
                try:
                    ...
                    print("caught error checking if the llm_assistant_process has died: ", ex)
                ...
                if self.llm_assistant_process.poll() == None:
                                ...
                                with open(str(self.llm_assistant_output_file_path), "r") as f:
                                    ...
                                    if len(in_suffix) > 1 and full.startswith(in_suffix):
                                        ...
                                        if len(original_voice_message) > 1 and len(before_assistant) > 3 and len(before_assistant) > len(in_suffix) and not str(original_voice_message[:-1]) in before_assistant:
                                            ...
                                            self.speak("Sorry, the AI assistant was hallucinating. It's restarting.", intent)
                                            self.force_reset_assistant()
                                            return
                                    ...
                                    if ((origin == 'voice' and '.' in full or ', ' in full or '?' in full or '!' in full or ':' in full and self.controller_pi_version > 4) or
                                        ...
                                        if display or (origin == 'voice' and self.controller_pi_version < 5) or self.llm_assistant_do_not_split_on_comma:
                                            pre_lines = re.split(r"([.?!:]+)", full)
                                        else:
                                        ...
                                        if display:
                                            ...
                                            for display_line in pre_lines:
                                                ...
                                                if in_suffix in display_line:
                                                    if self.DEBUG:
                                                        print("note: removing in_suffix from display_line: ->" + str(in_suffix) + "<-")
                    ...
                    if self.DEBUG:
                        print("really_ask_ai_assistant: YIKES, the assistant has already stopped")
